        ...
        const html = `
          <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
            ...
            <p style="color: #666; font-size: 14px; margin-top: 20px;">
              <a href="${process.env.NEXT_PUBLIC_APP_URL}/dashboard/crm/calendar" style="color: #6366f1; text-decoration: none;">
                View in Calendar →
              </a>
            </p>
          </div>
...
/**
 * GET /api/calendar/followups
 * Get follow-up status for events
 */
export async function GET(request: NextRequest) {
  try {
    ...
    let query = supabase
      .from('calendar_events')
      .select('id, title, end_time, follow_up_enabled, follow_up_triggered, follow_up_delay_hours')
      .eq('user_id', user.id)
      .eq('follow_up_enabled', true)
    ...
    console.error('Error in GET /api/calendar/followups:', error)
    return NextResponse.json(
      { error: 'Internal server error', details: error.message },
      { status: 500 }
    )
